...
import { Button, Card, Chip, PosterImage, Screen, SectionHeader, Spinner, StatusChip, T, WatchButton } from '../../components/primitives';
...
export default function EpisodeDetailScreen() {
  ...
  const { data: ep, isLoading } = useEpisode(id);
  ...
  if (isLoading || !ep) return <Screen><Header showBack /><Spinner /></Screen>;
  ...
  const voteCharacter = async (characterName?: string | null) => {
    ...
    try {
      await api.post(`/episodes/${id}/character-vote`, { characterName });
  ...
  return (
    <Screen>
      <ScrollView showsVerticalScrollIndicator={false}>
        <ImageBackground source={{ uri: ep.showImages.backdrop ?? ep.stillUrl ?? undefined }} style={styles.hero} imageStyle={{ opacity: 0.6 }}>
          <View style={styles.overlay}>
            <Header showBack right={<Pressable hitSlop={10}><Ionicons name="share-outline" size={22} color={colors.text} /></Pressable>} />
            <View style={{ padding: spacing.lg }}>
              ...
              <T variant="caption" style={{ color: colors.textMuted, marginTop: spacing.sm }}>
                S{String(ep.seasonNumber).padStart(2, '0')} | E{String(ep.number).padStart(2, '0')}
              </T>
              <T variant="title" style={{ fontSize: 24 }}>{ep.title}</T>
            </View>
          </View>
        </ImageBackground>
        ...
        <View style={{ paddingHorizontal: spacing.lg, gap: spacing.lg, marginTop: spacing.md }}>
          <Card style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <WatchButton
              watched={!!ep.watched}
              size={44}
              onPress={() => mark.mutate({ id, on: !ep.watched, ...(rating ? { rating } : {}) })}
            />
          </Card>
            ...
            <>
              <Card>
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                  {DEVICES.map((d) => (
                    <Pressable key={d.key} onPress={() => setDevice(d.key)} style={[styles.deviceBtn, device === d.key && { borderColor: colors.primary }]}>
